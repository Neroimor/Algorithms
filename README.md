...
Стандартный алгоритм возведения в степень — это простой способ вычисления степени числа, который умножает число само на себя заданное количество раз. Например, для возведения числа `x` в степень `n` мы просто умножаем `x` на себя `n` раз.

Степень (n) - целое число
...
### Быстрое возведение в степень (Fast exponentiation algorithm)

Быстрое возведение в степень — это алгоритм, который использует свойства степеней для ускорения вычислений. Он работает по принципу деления степени на 2 и умножения результата на себя. Если степень нечётная, то мы умножаем ещё на основание.

#### Рексурсивный способ
Рекурсивный алгоритм возведения в степень работает следующим образом:

Шаги:

1) Если степень равна 0, возвращаем 1 (любое число в нулевой степени равно 1).
2) Если степень равна 1, возвращаем основание (любое число в первой степени равно самому себе).
3) Если степень чётная, делим её на 2 и рекурсивно вызываем функцию для половинной степени, затем умножаем результат на себя.
4) Если степень нечётная, делим её на 2, рекурсивно вызываем функцию для половинной степени, затем умножаем результат на себя и ещё раз на основание.
...
Итеративный алгоритм возведения в степень работает следующим образом:
Шаги:
1) Инициализируем переменную `result` равной 1.
2) Пока степень больше 0:
   - Если степень нечётная, умножаем `result` на основание/ степень уменьшается на 1.
   - Если степень чётная, делим её на 2 и умножаем основание на себя.
